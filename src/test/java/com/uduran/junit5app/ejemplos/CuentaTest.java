...
class CuentaTest {
    ...
    void initTestMethod(TestInfo testInfo, TestReporter testReporter){
        ...
        testReporter.publishEntry("Ejecutando " + testInfo.getDisplayName() + " "
                + testInfo.getTestMethod().orElse(null).getName() + " con las etiquetas "
                + testInfo.getTags());
    }
    ...
    class ParameterizedCuentaTest{
        ...
        @ParameterizedTest(name = "numero {index} ejecutando con valor {0} - {argumentsWithNames}")
        @CsvSource({"200, 100, John, Andres", "250, 200, Pepe, Pepe", "299, 300, maria, Maria", "510, 500, Ulises, Ulises", "750, 700, Estela, estela", "2000, 2000, Mariana, Mariana"})
